...
def winning_team
   ...
   home_sum = 0
   away_sum = 0
   temp[:home][:players].each {|n| home_sum += n[:points]}
   temp[:away][:players].each {|n| away_sum += n[:points]}
   return temp[:home][:team_name] if home_sum > away_sum
